...
function Homestays() {
  const { category } = useParams();   // homestays / events / heritage
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_BACKEND_URL}/products?category=${category}`)
      .then((res) => res.json())
      .then((data) => {
        setPlaces(data);   // API already filtered, no need extra filter
      })
  ...
  const handleDelete = async (id) => {
    ...
    await fetch(`${process.env.REACT_APP_BACKEND_URL}/products/${id}`, {
      method: "DELETE",
    });
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mt-3 text-primary">
                  <MapPin size={18} />
                  <span>{place.location || "Location not available"}</span>
                </div>
